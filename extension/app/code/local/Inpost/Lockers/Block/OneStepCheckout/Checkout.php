...
class Inpost_Lockers_Block_OneStepCheckout_Checkout extends Idev_OneStepCheckout_Block_Checkout {
    ...
    public function _handlePostData()
    {
        ...
        if (strpos($this->getQuote()->getShippingAddress()->getShippingMethod(), 'inpost_lockers') !== false &&
            strpos(strtolower($this->getQuote()->getShippingAddress()->getCompany()), 'inpost') !== false) {
            $shippingAddress = $this->getQuote()->getShippingAddress();
            $billing_data['use_for_shipping'] = 0;
            $result = $this->getOnepage()->saveBilling($billing_data, $customerAddressId);
            $this->getQuote()->getShippingAddress()->setShippingMethod($shippingAddress->getShippingMethod())->save();
        } else {
            if($this->differentShippingAvailable()) {
